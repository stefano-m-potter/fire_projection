...
#             * Model file
#             * Row in summary CSV with IoU/F1 + ROC AUC + PR AUC
# """
...
# OUT_DIR  = os.path.join(OUT_ROOT, "option4_focal_loss_10x_negative_auc")
...
#         OUT_DIR,
#         "option4_focal_globaltest_neg_fraction_sweep_metrics_auc.csv"
#     )
#     summary_df.to_csv(summary_csv, index=False)
#     print(f"\nSaved Option 4 (focal) neg-fraction sweep summary with AUCs to: {summary_csv}")
# else:
#     print("\nNo neg-fraction runs were executed; summary not saved.")



#_----with learning curves
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
- Step 2: On the TrainVal pool ONLY:
    * For each negative fraction f in [1.0, 0.9, ..., 0.1]:
        ...
        - Sample f * (all negatives in TrainVal)
        - Stratified Train vs Val split
        - Train focal-loss model for a fixed 100 trees (no early stopping)
        - Track IoU per iteration on train & val
        - Define best_iteration as the iteration with max val IoU
        - Pick threshold to MAXIMIZE F1 on validation
        - Evaluate on the SAME fixed test set using best_iteration
        - Save:
            * IoU learning curve PNG
            * Feature importance PNG
            * ROC curve PNG (with AUC)
            * Precision–Recall curve PNG (with AUPRC)
            * Model file (with metadata in CSV)
...
for step_idx, frac in enumerate(NEG_FRAC_STEPS):
    ...
    evals_result = {}
    backend = "lightgbm"
    best_iter = NUM_BOOST_ROUNDS  # default, will be overwritten if we have IoU
    ...
    if USE_LGB_FOBJ:
        ...
        # ---- IoU learning curve + best iteration from val IoU ----
        if "IoU" in evals_result.get("validation", {}):
            train_iou_curve = evals_result["train"]["IoU"]
            val_iou_curve   = evals_result["validation"]["IoU"]

            # best iteration = argmax val IoU (1-based indexing for iteration)
            best_iter = int(np.argmax(val_iou_curve) + 1)

            plt.figure(figsize=(8, 5))
            plt.plot(train_iou_curve, label="Train IoU (focal)")
            plt.plot(val_iou_curve, label="Validation IoU (focal)")
            plt.xlabel("Boosting Rounds")
            plt.ylabel("IoU (Jaccard)")
            plt.title(
                f"Option 4 (Focal-LGB): Train vs Val IoU\n"
            ...
            # fallback if IoU missing
            best_iter = NUM_BOOST_ROUNDS

        # Validation probabilities using best_iter
        y_val_proba = sigmoid(
            booster.predict(X_val, num_iteration=best_iter)
        )
        ...
        if "IoU" in evals_result.get("validation", {}):
            ...
            plt.ylabel("IoU (Jaccard)")
            plt.title(
                f"Option 4 (Focal-XGB): Train vs Val IoU\n"
                f"Neg fraction={frac:.1f} (≈{eff_ratio:.2f}:1 in TrainVal subset)"
            )
            plt.legend()
            plt.grid(True)
            ...
            plt.savefig(iou_fig_out, dpi=150)
            plt.close()
            print(f"Saved focal IoU curve (XGB): {iou_fig_out}")
        else:
            best_iter = NUM_BOOST_ROUNDS

        # Validation probabilities (margins -> probs) using best_iter
        y_val_proba = sigmoid(
            booster.predict(dval, iteration_range=(0, best_iter))
        )
    ...
    if USE_LGB_FOBJ:
        y_test_proba = sigmoid(
            booster.predict(X_test, num_iteration=best_iter)
        )
    else:
        import xgboost as xgb
        dtest = xgb.DMatrix(X_test, enable_categorical=True)
        y_test_proba = sigmoid(
            booster.predict(dtest, iteration_range=(0, best_iter))
        )
    ...
    # Save the booster itself; best_iter is stored in the CSV.
    joblib.dump(booster, model_path)
    print(f"Saved model for neg fraction {frac:.1f} to: {model_path}")
