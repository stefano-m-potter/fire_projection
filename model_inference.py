...
# For each model trained in:
#   .../ml_training/neg_ratio_experiments_globaltest/option4_focal_loss_10x_negative/

# we use:
#   - models/focal_model_negfrac{neg_fraction_pct:03d}_{backend}.pkl
#   - option4_focal_globaltest_neg_fraction_sweep_metrics.csv

# Each row in the summary CSV gives:
#   - neg_fraction, neg_fraction_pct, backend
#   - threshold (probability)
#   - best_iteration

# We also reconstruct the predictor list from the 10x balanced training file:
...
#         predictions_option4_focal_10x_negative_negfrac{pct:03d}_mcd/
...
# # Focal training output directory (models + summary CSV)
# FOCAL_OUT_DIR = (
...
#     FOCAL_OUT_DIR, "option4_focal_globaltest_neg_fraction_sweep_metrics.csv"
# )
...
# print(summary_df[["neg_fraction", "neg_fraction_pct", "backend", "threshold"]])
...
#         PRED_ROOT,
#         f"predictions_option4_focal_10x_negative_negfrac{neg_fraction_pct:03d}_mcd"
#     )
...
# print("\n✅ Done. All requested predictions written for all neg-fraction focal models.")
...
"""
Predict burned probabilities and classes for the BEST-PRECISION
BalancedBagging + XGBoost model (neg_pos_target = 100).
...
Training outputs:
  ...
  Models:
    OUT_DIR/models/balancedbagging_xgb_model_negpos100.pkl

  Summary CSV:
    OUT_DIR/balancedbagging_trueTest_train100_negpos_sweep_metrics_auc_thresh.csv

We:
  - Recover predictor list from the Train/Val 100x parquet
    (/explore/nobackup/people/spotter5/clelland_fire_ml/parquet_cems_trainval_100x)
  - Load the neg_pos_target=100 row from the summary CSV
    to get the probability threshold
  - Predict from the full partitioned Parquet dataset:

    /explore/nobackup/people/spotter5/clelland_fire_ml/parquet_cems_with_fraction_dataset_mcd

  - For each YEAR × MONTH:
        * read features from Parquet
        * run BalancedBagging+XGB predict_proba
        * write rasters to:

    /explore/nobackup/people/spotter5/clelland_fire_ml/
        predictions_balancedbagging_negpos100_mcd/
            proba/cems_pred_proba_{YYYY}_{MM}.tif
            class/cems_pred_class_{YYYY}_{MM}_thr{thr}.tif
"""
...
# Train/Val 100x parquet directory used to train BalancedBagging models
TRAINVAL_100X_DIR = "/explore/nobackup/people/spotter5/clelland_fire_ml/parquet_cems_trainval_100x"

# BalancedBagging training output directory (models + summary CSV)
BB_OUT_ROOT = "/explore/nobackup/people/spotter5/clelland_fire_ml/ml_training/neg_ratio_experiments_globaltest"
BB_OUT_DIR  = os.path.join(BB_OUT_ROOT, "balancedbagging_trueTest_train100_negpos_sweep")

MODELS_DIR  = os.path.join(BB_OUT_DIR, "models")
SUMMARY_CSV = os.path.join(
    BB_OUT_DIR, "balancedbagging_trueTest_train100_negpos_sweep_metrics_auc_thresh.csv"
)

# Root where prediction rasters for this model will be stored
PRED_ROOT = "/explore/nobackup/people/spotter5/clelland_fire_ml"
OUT_PRED_DIR = os.path.join(PRED_ROOT, "predictions_balancedbagging_negpos100_mcd")
...
# The neg_pos_target we want (BEST PRECISION)
BEST_NEG_POS_TARGET = 100
...
# ============================================================
# 1) Recover predictor list from the Train/Val 100x parquet
# ============================================================

print(f"Reading Train/Val 100x parquet from:\n  {TRAINVAL_100X_DIR}")
tv_dataset = ds.dataset(TRAINVAL_100X_DIR, format="parquet")
# Just sample some rows to get columns
tv_table = tv_dataset.to_table().slice(0, 1000)
df_train_meta = tv_table.to_pandas()

if "fraction" not in df_train_meta.columns:
    raise ValueError("Expected column 'fraction' in Train/Val 100x dataset.")

drop_cols = {"fraction", "burned", "bin", "year", "month", "latitude", "longitude"}
predictors = [c for c in df_train_meta.columns if c not in drop_cols]

print("\nRecovered predictor columns from Train/Val 100x parquet:")
print(f"  #predictors: {len(predictors)}")
print("  First few predictors:", predictors[:10])

# ============================================================
# 2) Load summary CSV and extract threshold for neg_pos_target=100
# ============================================================
...
row_100 = summary_df.loc[summary_df["neg_pos_target"] == BEST_NEG_POS_TARGET]
if row_100.empty:
    raise ValueError(
        f"No row with neg_pos_target={BEST_NEG_POS_TARGET} in {SUMMARY_CSV}"
    )

THRESH_PROBA = float(row_100["threshold"].iloc[0])
print(f"\nUsing best-precision model with neg_pos_target={BEST_NEG_POS_TARGET}")
print(f"  threshold (proba) from summary CSV: {THRESH_PROBA:.3f}")

# ============================================================
# 3) Load the BalancedBagging+XGB model for negpos100
# ============================================================

model_path = os.path.join(
    MODELS_DIR,
    f"balancedbagging_xgb_model_negpos{BEST_NEG_POS_TARGET:03d}.pkl"
)
if not os.path.exists(model_path):
    raise FileNotFoundError(f"Model file not found: {model_path}")

print(f"\nLoading BalancedBagging+XGB model from:\n  {model_path}")
...
# ============================================================
# 4) Prepare Parquet dataset handle once
# ============================================================
...
# ============================================================
# 5) Predict for requested YEARS × MONTHS
# ============================================================
...
for YEAR in YEARS:
    for MONTH in MONTHS:
        ...
        print(f"\n[negpos{BEST_NEG_POS_TARGET:03d}] === Predicting {YEAR}-{MONTH:02d} ===")
        ...
        with rio.open(template_path) as tpl:
            ...
            for batch in scanner.to_batches():
                ...
                # Predict probabilities with BalancedBagging+XGB
            ...
            out_proba = Path(OUT_PRED_DIR) / "proba" / f"cems_pred_proba_{YEAR}_{MONTH:02d}.tif"
            out_class = (
                Path(OUT_PRED_DIR) / "class" /
                f"cems_pred_class_{YEAR}_{MONTH:02d}_thr{THRESH_PROBA:.3f}.tif"
            )

            write_geotiff(str(out_proba), proba_arr, tpl, nodata=np.nan, dtype="float32")
            write_geotiff(str(out_class), class_arr, tpl, nodata=255, dtype="uint8")

            print(f"[negpos{BEST_NEG_POS_TARGET:03d} {YEAR}-{MONTH:02d}] "
                  f"wrote {written_rows:,}/{total_rows:,} rows ->")
            print(f"  - {out_proba}")
            print(f"  - {out_class}")

print("\n✅ Done. Predictions written for best-precision BalancedBagging+XGB model (negpos100).")
